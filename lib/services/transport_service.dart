...
class TransportService {
  TransportService(this._db, this._identityService, this._securityService, this._fileStore);

  final AppDatabase _db;
  final IdentityService _identityService;
  final SecurityService _securityService;
  final FileStore _fileStore;
  final _dio = dio.Dio(dio.BaseOptions(connectTimeout: const Duration(seconds: 5)));
  ...
  Future<Response> _pairRequest(Request request) async {
    ...
    if (deviceId is! String || code is! String) return Response.badRequest(body: 'Invalid pair request');
  ...
  Future<void> pairWith(Device peer, String code) async {
    if (peer.host == null || peer.port == null) throw StateError('Peer endpoint is not known.');
    final identity = _identityService.identity;
    ...
    await _db.trustDevice(
      ...
      host: peer.host!,
      port: peer.port!,
      signingPublicKey: _string(data['signing_public_key'], peer.signingPublicKey),
      exchangePublicKey: _string(data['exchange_public_key'], peer.exchangePublicKey),
  ...
  Future<void> sendText(Device peer, String text) async {
    ...
    _updates.add(null);
    try {
      await _postSecure(peer, '/v1/messages', {
        'id': messageId,
        'kind': 'text',
        'body': text,
  ...
  Future<void> _sendFile(Device peer, File file) async {
    ...
    await _db.into(_db.transfers).insertOnConflictUpdate(
    ...
    try {
      ...
      await for (final part in stream) {
        ...
        while (buffer.length >= defaultTransferChunkSize) {
          ...
          await _sendChunk(peer, transferId, index, Uint8List.fromList(chunk.take(defaultTransferChunkSize).toList()));
      ...
      await _postSecure(peer, '/v1/transfers/$transferId/complete', {'id': transferId, 'sha256': sha});
  ...
  Future<void> _sendChunk(Device peer, String transferId, int index, List<int> bytes) async {
  ...
  Future<Response> _receiveMessage(Request request) => _withTrustedEnvelope(request, (peer, payload) async {
    final conversation = await _db.ensureConversation(peer);
    await _db.addMessage(
      ChatMessagesCompanion.insert(
        ...
        status: 'received',
        createdAt: DateTime.tryParse(_string(payload['created_at'], '')) ?? DateTime.now(),
      ),
    );
    _updates.add(null);
  ...
  Future<Response> _receiveTransferStart(Request request) => _withTrustedEnvelope(request, (peer, payload) async {
    final file = await _fileStore.createReceiveFile(_string(payload['file_name'], 'received.bin'));
    ...
    await _db.addMessage(
      ChatMessagesCompanion.insert(
        ...
        direction: 'in',
        kind: 'file',
        fileName: Value(_string(payload['file_name'], p.basename(file.path))),
        filePath: Value(file.path),
  ...
  Future<Response> _receiveTransferChunk(Request request, String id, String index) =>
      _withTrustedEnvelope(request, (peer, payload) async {
        final transfer = await (_db.select(_db.transfers)..where((tbl) => tbl.id.equals(id))).getSingleOrNull();
        if (transfer == null || transfer.filePath == null) return Response.notFound('Transfer not found');
  ...
  Future<Response> _withTrustedEnvelope(
    ...
    if (sender is! String) return Response.forbidden('Missing sender');
    final peer = await _db.getDevice(sender);
    if (peer == null || !peer.trusted) return Response.forbidden('Peer is not trusted');
    final payload = await _securityService.open(peer, json);
    return body(peer, payload);
  }

  Future<void> _postSecure(Device peer, String path, Map<String, Object?> payload, {String method = 'post'}) async {
    if (peer.host == null || peer.port == null) throw StateError('Peer endpoint is not known.');
    final envelope = await _securityService.seal(peer, payload);
  ...
  Future<void> _markTransferStatus(String id, String status, int receivedBytes) async {
    await (_db.update(_db.transfers)..where((tbl) => tbl.id.equals(id))).write(
      TransfersCompanion(
        status: Value(status),
        receivedBytes: Value(receivedBytes),
