...
void main() {
  ...
  test('new identity stores private keys in secure storage, not DB plaintext', () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final identity = IdentityService(db, secureKeyStore: const SecureKeyStore());
  ...
  test('legacy plaintext private keys migrate to secure storage and are cleared', () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    ...
    final migrated = IdentityService(db, secureKeyStore: const SecureKeyStore());
  ...
  test('identity is stable across reloads after migration', () async {
    final file = File('${Directory.systemTemp.path}/localchat-secure-${DateTime.now().microsecondsSinceEpoch}.sqlite');
    addTearDown(() {
